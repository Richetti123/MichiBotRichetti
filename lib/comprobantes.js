...
export async function handleIncomingMedia(m, conn) {
    ...
    // <--- NUEVOS LOGS AQUÍ PARA INSPECCIONAR m.message y m.sender --->
    console.log('DEBUG COMPROBANTES: Valor de m.message:', m.message);
    console.log('DEBUG COMPROBANTES: Valor de m.sender:', m.sender);
    // ----------------------------------------------------------------
    ...
    if (!m.message || m.sender === conn.user.jid || !m.sender) { 
        console.log('DEBUG COMPROBANTES: Condición de salida temprana cumplida. m.message, m.sender, o bot self-message.'); // LOG ADICIONAL
        console.log('--- DEBUGGING: FIN COMPROBANTES.JS ---\n'); // Cerrar log al salir temprano
        return false;
    }
    
    // --- CONTINUACIÓN DE LOGS DE DEPURACIÓN ---
    console.log('Mensaje entrante del remitente:', m.sender);
    console.log('Tipo de mensaje (m.mtype):', m.mtype);
    // --- FIN CONTINUACIÓN DE LOGS DE DEPURACIÓN ---
    ...
    if (isMedia && isPaymentProof(captionText)) {
        ...
        try {
            ...
            console.log('--- DEBUGGING: FIN COMPROBANTES.JS ---\n'); // Cerrar log al finalizar correctamente
            return true; 
        } catch (e) {
