...
export async function handleIncomingMedia(m, conn) {
    ...
    console.log('DEBUG COMPROBANTES: Valor de m.sender (Original):', m.sender); // Mantener este log para referencia

    // --- NUEVA LÓGICA PARA EXTRAER EL REMITENTE DE FORMA ROBUSTA ---
    // m.key.remoteJid es el JID del chat (grupo o individual)
    // m.key.participant es el JID del participante en un grupo
    // m.key.fromMe indica si el mensaje es del propio bot
    ...
    // --- CONDICIÓN DE SALIDA TEMPRANA REFINADA ---
    // Salir si no hay contenido de mensaje, si no se pudo extraer un remitente, o si el mensaje es del propio bot.
    ...
    // Ahora, usa extractedSenderJid para el resto de la lógica
    const senderJid = extractedSenderJid; 
    const senderNumber = senderJid.split('@')[0]; 
    const formattedSenderNumber = `+${senderNumber}`; 

    console.log('Mensaje entrante del remitente:', senderJid); // Usar el senderJid extraído
    ...
    // La forma de obtener el caption también es importante si el tipo de mensaje varía
    const captionText = m.message.imageMessage?.caption || 
                        m.message.videoMessage?.caption || 
                        m.message.documentMessage?.caption || 
                        m.message.extendedTextMessage?.text || // Para mensajes de texto con enlaces o citas
                        '';
